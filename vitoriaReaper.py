...
class ReaperFinalizacao(pygame.sprite.Sprite):
    def __init__(self, posX, posY):
        super().__init__()
        self.frames = [
            pygame.image.load('graficos/FINALIZACOES/REAPER/1f.png').convert_alpha(),
            pygame.image.load('graficos/FINALIZACOES/REAPER/2f.png').convert_alpha(),
            pygame.image.load('graficos/FINALIZACOES/REAPER/3f.png').convert_alpha(),
            pygame.image.load('graficos/FINALIZACOES/REAPER/4f.png').convert_alpha(),
            pygame.image.load('graficos/FINALIZACOES/REAPER/5f.png').convert_alpha(),
            pygame.image.load('graficos/FINALIZACOES/REAPER/6f.png').convert_alpha(),
        ...
        self.tempo_frame = 0.3  # Tempo de exibição de cada frame em segundos
...
def init(game):#ANIMADO
    #SONS
    pygame.mixer.stop()
    musica_fundo('sounds/REAPER/reaperbackground.ogg',0.10)# loop
    musica_rounds('sounds/Victory Sound.ogg',0.08)#som vitoria
    musica_rounds('sounds/REAPER/reaperfrase.ogg',0.50)#frase

    #ANIMADO
    game.moving_finalizacao = pygame.sprite.Group()
    game.fundo = ReaperFinalizacao(0, 0)
