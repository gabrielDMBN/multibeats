...
class SukunaFinalizacao(pygame.sprite.Sprite):
    def __init__(self, posX, posY):
        super().__init__()
        self.frames = [
            pygame.image.load('graficos/FINALIZACOES/SUKUNA/1f.png').convert_alpha(),
            pygame.image.load('graficos/FINALIZACOES/SUKUNA/2f.png').convert_alpha(),
            pygame.image.load('graficos/FINALIZACOES/SUKUNA/3f.png').convert_alpha(),
            pygame.image.load('graficos/FINALIZACOES/SUKUNA/2f.png').convert_alpha(),
        ]
        self.frame_atual = 0
...
def init(game):#ANIMADO
    #SONS
    pygame.mixer.stop()
    musica_fundo('sounds/SUKUNA/sukunabackground.ogg',0.04) #loop
    musica_rounds('sounds/Victory Sound.ogg',0.08) #som vitoria
    musica_rounds('sounds/SUKUNA/sukunafrase.ogg',0.25) #frase

    #ANIMADO
    game.moving_finalizacao = pygame.sprite.Group()
    game.fundo = SukunaFinalizacao(0, 0)
