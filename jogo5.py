...
def init(game):
    ...
    musica_rounds('musicas/round5.ogg',0.18)###################
    ...
    args = (
        (1, pygame.K_a, game, 180),
        (2, pygame.K_s, game, 180),
        (3, pygame.K_d, game, 180),
        (4, pygame.K_LEFT, game, 180),
        (5, pygame.K_DOWN, game, 180),
        (6, pygame.K_RIGHT, game, 180)
    )
    game.notas = []
    rounds = import_module(f'round{5}')#############
    for notas in rounds.Notas:
        nota_linha = []
        for val, i in enumerate(notas):
            if i != 0:
                nota_linha.append(BlocoNotas(*args[val], i*100))
        game.notas.append(nota_linha)
...
def loop(game):
    ...
    if game.tempo>1:
        ...
        if game.pontuacaoE>game.pontuacaoD:
            game.HPD -= 1
            game.new_level('intervalofinal')
        else:
            game.HPE -= 1
            game.new_level('intervalofinal')
    ...
    #PERSONAGENS
    #game.screen.blit(game.personagemE,(590,565))
    game.moving_sprite2.draw(game.screen)
    game.moving_sprite2.update()
    #game.screen.blit(game.personagemD, (1171, 565))
    ...
    for linha, notas in enumerate(game.notas):
        if 90 - linha < game.tempo:
            break
        for nota in notas:
            nota.update()
    ...
    #definindo rounds
    rounds = BIG.render(f'Round {5}', False, ('White')).convert_alpha()
